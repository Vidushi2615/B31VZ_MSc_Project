...
int cx = static_cast<int>(detection.bbox.center.x);
int cy = static_cast<int>(detection.bbox.center.y);

if (cx < 0 || cy < 0 || cx >= width || cy >= height) {
ROS_WARN("Bounding box center is outside image bounds.");
return false;
}

// Lookup the point in the point cloud
pcl::PointXYZ pt = cloud.at(cx, cy);

if (!std::isfinite(pt.x) || !std::isfinite(pt.y) || !std::isfinite(pt.z)) {
ROS_WARN("Invalid 3D point at bounding box center.");
return false;
}
...
ROS_INFO_STREAM("3D point found: [" << pt.x << ", " << pt.y << ", " << pt.z << "]");
return true;
}
